...
$backup = Join-Path (Split-Path $root -Parent) "almustafa-connect-erp_backups\school_store_phase2_dashboard_fix_v2_$stamp"
...
if (-not $text.Contains("import 'store_purchases_page.dart';")) {
  $importAnchor = "import '../bloc/school_store_state.dart';"
  ...
  $text = $text.Replace(
    $importAnchor,
    "$importAnchor`nimport 'store_purchases_page.dart';"
  )
}
...
"@
  ...
  if (-not $text.Contains($target)) {
    throw 'DASHBOARD INSERTION ANCHOR ERROR.'
  }
