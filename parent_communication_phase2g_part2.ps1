...
$backup=Join-Path (Split-Path $root -Parent) "almustafa-connect-erp_backups\parent_chat_phase2g_part2_$stamp"
...
class _ParentChatViewState extends State<_ParentChatView> {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<ChatBloc, ChatState>(
      ...
      builder: (context, state) {
        ...
        final threads = loaded.threads.where((thread) {
          ...
          final teacherName = _otherParticipantName(thread).toLowerCase();
  ...
  String _otherParticipantName(ChatThreadEntity thread) {
    for (final entry in thread.participantNames.entries) {
      if (entry.key != widget.parentUserId &&
  ...
  Future<void> _createTeacherThread(BuildContext context) async {
    ...
    try {
      final accounts =
          await UserAccountServiceImpl().listChatParticipants();
      teachers = accounts
          .where(
            (account) =>
    ...
    context.read<ChatBloc>().add(
          CreateChatThreadRequested(
            ChatThreadEntity(
              id:
                  'parent_teacher_${widget.parentUserId}_${teacher.uid}_${widget.student.id}',
...
Write-Host ''
Write-Host 'Parent Communication Phase 2G Part 2 completed successfully.' -ForegroundColor Green
Write-Host 'Parent Portal V1 communication module is now complete.' -ForegroundColor Green
