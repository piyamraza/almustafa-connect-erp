...
$backup=Join-Path (Split-Path $root -Parent) "almustafa-connect-erp_backups\parent_attendance_phase2b_part2_$stamp"
...
$page='lib/features/parent_portal/presentation/pages/parent_attendance_page.dart'
...
class ParentAttendancePage extends StatefulWidget {
  ...
  @override
  State<ParentAttendancePage> createState() =>
      _ParentAttendancePageState();
}

class _ParentAttendancePageState
    extends State<ParentAttendancePage> {
  final ParentAttendanceService _service =
      sl<ParentAttendanceService>();
...
class _AttendancePercentageCard extends StatelessWidget {
  const _AttendancePercentageCard({
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      child: ListTile(
        ...
        subtitle: record.remarks.trim().isEmpty
            ? null
            : Text(record.remarks),
        trailing: Chip(
...
$import="import 'parent_attendance_page.dart';"
if(-not $text.Contains($import)){
  $anchor="import 'parent_academic_dashboard_page.dart';"
...
if(-not $text.Contains($anchor2)){
  throw 'Attendance navigation anchor not found.'
}
...
flutter analyze lib/features/parent_portal lib/features/attendance --no-fatal-infos --no-fatal-warnings
if($LASTEXITCODE -ne 0){throw "ANALYZE ERROR. Backup: $backup"}

Write-Host ''
Write-Host 'Parent Attendance Phase 2B completed successfully.' -ForegroundColor Green
