...
$backup=Join-Path (Split-Path $root -Parent) "almustafa-connect-erp_backups\audit_viewer_integration_$stamp"
...
function WriteText([string]$p,[string]$text){
  $target=Full $p
  New-Item -ItemType Directory -Force -Path (Split-Path $target -Parent)|Out-Null
  [IO.File]::WriteAllText($target,$text.Replace("`r`n","`n"),$utf8)
}

if(-not(Test-Path (Full 'pubspec.yaml'))){throw 'Run from project root.'}

$page='lib/core/audit/presentation/pages/audit_logs_page.dart'
      ...
      child: Wrap(
        ...
        children: [
          ...
          OutlinedButton.icon(
            ...
            label: Text(
              _fromDate == null ? 'From Date' : _date(_fromDate!),
            ),
          ),
          OutlinedButton.icon(
...
$text=[IO.File]::ReadAllText((Full $sidebar)).Replace("`r`n","`n")

$importAnchor="import '../../../access_control/presentation/pages/unauthorized_access_page.dart';"
$auditImport="import '../../../../core/audit/presentation/pages/audit_logs_page.dart';"

if(-not $text.Contains($auditImport)){
  if(-not $text.Contains($importAnchor)){throw 'Sidebar import anchor not found.'}
  $text=$text.Replace($importAnchor,$auditImport+"`n"+$importAnchor)
}
...
if(-not $text.Contains("title: 'Audit Logs'")){
  if(-not $text.Contains($menuAnchor)){throw 'Sidebar settings menu anchor not found.'}
  $text=$text.Replace($menuAnchor,$auditMenu+$menuAnchor)
}

WriteText $sidebar $text

dart format $page $sidebar
if($LASTEXITCODE -ne 0){throw "FORMAT ERROR. Backup: $backup"}

flutter analyze lib/core/audit lib/features/dashboard lib/features/access_control --no-fatal-infos --no-fatal-warnings
if($LASTEXITCODE -ne 0){throw "ANALYZE ERROR. Backup: $backup"}

Write-Host ''
Write-Host 'Audit Viewer Integration completed successfully.' -ForegroundColor Green
Write-Host 'Audit Logs menu is now available to authorized users.' -ForegroundColor Green
Write-Host 'Viewer includes module, action, search and date filters.' -ForegroundColor Green
