...
$backup = Join-Path (Split-Path $root -Parent) "almustafa-connect-erp_backups\teacher_audit_integration_$stamp"
...
class TeacherRepositoryImpl implements TeacherRepository {
  ...
  Map<String, dynamic> _auditValues(TeacherModel model) {
    ...
    // CNIC is intentionally excluded from the audit payload.
    values.remove('cnic');
...
$text = [IO.File]::ReadAllText((Full $serviceLocator)).Replace("`r`n", "`n")
...
if (-not $text.Contains($oldRegistration)) {
  throw 'TeacherRepository registration block not found in service_locator.dart.'
}

$text = $text.Replace($oldRegistration, $newRegistration)
WriteText $serviceLocator $text

Write-Host ''
Write-Host 'Teacher Audit Integration files updated successfully.' -ForegroundColor Green
Write-Host 'Teacher create, update and delete actions will now generate audit logs.' -ForegroundColor Green
Write-Host 'Teacher CNIC is excluded from audit old/new value payloads.' -ForegroundColor Green
