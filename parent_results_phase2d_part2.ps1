...
$backup=Join-Path (Split-Path $root -Parent) "almustafa-connect-erp_backups\parent_results_phase2d_part2_$stamp"
...
class ParentResultsPage extends StatefulWidget {
  ...
  @override
  State<ParentResultsPage> createState() =>
      _ParentResultsPageState();
}

class _ParentResultsPageState extends State<ParentResultsPage> {
  final ParentResultsService _service =
      sl<ParentResultsService>();
    ...
    return ListView(
      ...
      children: [
        ...
        if (summary.results.isEmpty)
          ...
          const SizedBox(height: 16),
          if (selected != null)
            _ResultDetailCard(result: selected),
        ],
      ],
    );
  }
...
class _ResultStats extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final items = [
      ...
      (
        'Obtained',
        result.grandObtainedMarks.toStringAsFixed(1),
      ),
...
$import="import 'parent_results_page.dart';"
if(-not $text.Contains($import)){
  $anchor="import 'parent_homework_page.dart';"
...
if(-not $text.Contains("if (module.`$1 == 'Results')")){
  if(-not $text.Contains($homeworkBlock)){
    throw 'Homework navigation block not found.'
  }
  $text=$text.Replace($homeworkBlock,$resultsBlock)
}
...
Write-Host ''
Write-Host 'Parent Results Phase 2D completed successfully.' -ForegroundColor Green
