...
$backup=Join-Path (Split-Path $root -Parent) "almustafa-connect-erp_backups\parent_notices_phase2f_complete_$stamp"
...
$page='lib/features/parent_portal/presentation/pages/parent_notices_page.dart'
...
$import="import 'parent_notices_page.dart';"
if(-not $text.Contains($import)){
  $anchor="import 'parent_fee_page.dart';"
  if(-not $text.Contains($anchor)){
    $anchor="import 'parent_results_page.dart';"
  }
  if(-not $text.Contains($anchor)){
...
if(-not $text.Contains("if (module.`$1 == 'Notices')")){
  if(-not $text.Contains($feeBlock)){
    throw 'Fee navigation block not found.'
  }
  $text=$text.Replace($feeBlock,$noticeBlock)
}
...
flutter analyze lib/features/parent_portal lib/features/notices --no-fatal-infos --no-fatal-warnings
if($LASTEXITCODE -ne 0){throw "ANALYZE ERROR. Backup: $backup"}

Write-Host ''
Write-Host 'Parent Notices Phase 2F completed successfully.' -ForegroundColor Green
