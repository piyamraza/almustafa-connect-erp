...
class _ExamFormViewState extends State<_ExamFormView> {
  ...
  void _distributeDraftEqually(_SubjectDraft draft) {
    ...
    final total =
        double.tryParse(draft.totalController.text.trim())?.truncate() ?? 0;
    final passing =
  ...
  void _ensureComponentPassingMarks(_SubjectDraft draft) {
    if (draft.components.isEmpty) return;

    final hasAllPassingValues = draft.components.every((component) {
      final text =
          draft.componentPassingControllers[component.id]?.text.trim() ?? '';
      return double.tryParse(text) != null;
    });

    if (hasAllPassingValues) {
      _normalizeFractionalComponentMarks(draft);
      return;
    }
    ...
    final passing =
        double.tryParse(draft.passingController.text.trim())?.truncate() ?? 0;
    final values = _splitIntegerMarks(passing, draft.components.length);
  ...
  void _normalizeFractionalComponentMarks(_SubjectDraft draft) {
    if (draft.components.isEmpty) return;
    ...
    if (totalValues.any((value) => value != value.truncateToDouble())) {
      final normalized = _normalizeToIntegerTotal(totalValues, totalTarget);
      for (var index = 0; index < draft.components.length; index++) {
        draft.componentTotalControllers[draft.components[index].id]!.text =
    ...
    if (draft.componentPassingMode == _ComponentPassingMode.componentWise) {
      ...
      if (passingValues.any((value) => value != value.truncateToDouble())) {
        ...
        for (var index = 0; index < draft.components.length; index++) {
          draft.componentPassingControllers[draft.components[index].id]!.text =
              normalized[index].toString();
        }
      }
    }
  ...
  List<int> _splitIntegerMarks(int total, int count) {
    if (count <= 0) return const [];

    final safeTotal = total < 0 ? 0 : total;
    final base = safeTotal ~/ count;
    final remainder = safeTotal % count;
  ...
  List<int> _normalizeToIntegerTotal(List<double> values, int targetTotal) {
    ...
    final indexes = List<int>.generate(values.length, (index) => index)
      ..sort((first, second) {
        final firstFraction = values[first] - values[first].floor();
        final secondFraction = values[second] - values[second].floor();
        return secondFraction.compareTo(firstFraction);
      });

    var cursor = 0;
    while (remaining > 0) {
      floors[indexes[cursor % indexes.length]]++;
      cursor++;
      remaining--;
    }
    ...
    while (remaining < 0) {
      ...
      cursor--;
      if (cursor < 0) cursor = indexes.length - 1;
    }
...
String _marksText(double value) => value.truncate().toString();
