...
$backup=Join-Path (Split-Path $root -Parent) "almustafa-connect-erp_backups\parent_workspace_phase2_$stamp"
...
$file='lib/features/parent_portal/presentation/pages/parent_workspace_page.dart'
BackupFile $file
...
flutter analyze lib/features/parent_portal lib/features/dashboard lib/features/authentication --no-fatal-infos --no-fatal-warnings
if($LASTEXITCODE -ne 0){throw "ANALYZE ERROR. Backup: $backup"}

Write-Host ''
Write-Host 'Parent Workspace Phase 2 completed successfully.' -ForegroundColor Green
Write-Host 'All existing parent services are now connected to the parent login dashboard.' -ForegroundColor Green
Write-Host 'Multi-child selection refreshes the workspace automatically.' -ForegroundColor Green
