...
class _TeacherAttendanceViewState extends State<_TeacherAttendanceView> {
  ...
  void _save(List<TeacherEntity> teachers) {
    ...
    final existingRecords = switch (existing) {
      TeacherAttendanceLoaded(:final records) => records,
      TeacherAttendanceSaved(:final records) => records,
      _ => <TeacherAttendanceEntity>[],
    };
    final byTeacher = {
      for (final item in existingRecords) item.teacherId: item,
    };
  ...
  ) => BlocListener<TeacherAttendanceBloc, TeacherAttendanceState>(
    listener: (context, state) {
      if (state is TeacherAttendanceSaved) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              '${state.records.length} attendance record(s) saved.',
            ),
          ),
        );
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Unable to save attendance: ${state.message}'),
          ),
        );
      }
    },
              ...
              Expanded(
                child: BlocBuilder<TeacherAttendanceBloc, TeacherAttendanceState>(
                  builder: (context, state) {
                    ...
                    final previous = {
                      for (final item in records) item.teacherId: item.status,
                    };
