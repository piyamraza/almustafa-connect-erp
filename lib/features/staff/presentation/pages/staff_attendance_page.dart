...
class StaffAttendancePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider<StaffAttendanceBloc>(
      create: (_) =>
          sl<StaffAttendanceBloc>()
            ..add(LoadStaffAttendanceByDateEvent(DateTime.now())),
...
class _StaffAttendanceViewState extends State<_StaffAttendanceView> {
  ...
  Future<void> _pickDate() async {
    ...
    context.read<StaffAttendanceBloc>().add(
      LoadStaffAttendanceByDateEvent(_selectedDate),
    );
  }

  void _initializeExistingRecords(List<StaffAttendanceEntity> records) {
  ...
  Future<void> _saveAttendance(
    ...
    context.read<StaffAttendanceBloc>().add(
      SaveStaffAttendanceEvent(records: records, date: attendanceDate),
    );
  }
  ...
  void _markAllPresent(List<StaffEntity> activeStaff) {
    setState(() {
          ...
          child: FutureBuilder<List<StaffEntity>>(
            ...
            builder: (context, staffSnapshot) {
              ...
              if (staffSnapshot.hasError) {
                return _ErrorView(message: staffSnapshot.error.toString());
              }

              final activeStaff =
                  (staffSnapshot.data ?? const <StaffEntity>[])
              ...
              return BlocBuilder<StaffAttendanceBloc, StaffAttendanceState>(
                builder: (context, attendanceState) {
                  if (attendanceState is StaffAttendanceLoading && !_isSaving) {
                    return const Center(child: CircularProgressIndicator());
                  }

                  if (attendanceState is StaffAttendanceError && !_isSaving) {
                    return _ErrorView(
          ...
          builder: (context, constraints) {
            ...
            if (isCompact) {
              return Column(
                ...
                children: [information, const SizedBox(height: 16), actions],
              );
            }
