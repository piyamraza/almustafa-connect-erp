...
class ExamResultRemoteDataSourceImpl implements ExamResultRemoteDataSource {
  ...
  CollectionReference<Map<String, dynamic>> get _collection {
    return _service.collection(FirestorePaths.examResults);
  }
  ...
  Future<List<ExamResultModel>> getResultsForExam(String examId) async {
    ...
    final results = snapshot.docs
        .map(
          (document) =>
              ExamResultModel.fromMap({...document.data(), 'id': document.id}),
        )
  ...
  Future<List<ExamResultModel>> getPublishedResults({
    ...
    final snapshot = await _collection
        .where(
          'status',
          whereIn: [ResultStatus.published.name, ResultStatus.locked.name],
        )
        .get();
  ...
  Future<void> updateStatus({
    ...
    for (var start = 0; start < normalizedIds.length; start += 10) {
      ...
      final snapshot = await _collection
          .where(
            ...
            whereIn: normalizedIds.sublist(start, end),
          )
          .get();
    ...
    if (snapshots.length != normalizedIds.length) {
      throw StateError('One or more selected results no longer exist.');
    }
    ...
    final sourceStatuses = models.map((result) => result.status).toSet();
    if (sourceStatuses.length != 1) {
      throw StateError(
        'All selected results must have the same status before this action.',
      );
    }
    ...
    if (_requiresActor(status) && normalizedActorId.isEmpty) {
      throw StateError('${_statusLabel(status)} user ID is required.');
    }
    ...
    if (unlocking && normalizedReason.isEmpty) {
      throw StateError('Unlock reason is required.');
    }

    final now = DateTime.now();
    final nowText = now.toIso8601String();

    for (var start = 0; start < snapshots.length; start += 500) {
      ...
      for (final snapshot in snapshots.sublist(start, end)) {
        final current = ExamResultModel.fromMap({
          ...snapshot.data()!,
          'id': snapshot.id,
        });
  ...
  Map<String, dynamic> _statusUpdates({
    ...
    switch (next) {
      ...
      case ResultStatus.generated:
        updates.addAll({
          'generatedAt': current.generatedAt?.toIso8601String() ?? nowText,
          if (actorId.isNotEmpty) 'generatedBy': actorId,
        });

      case ResultStatus.verified:
        updates.addAll({'verifiedAt': nowText, 'verifiedBy': actorId});

      case ResultStatus.approved:
        updates.addAll({'approvedAt': nowText, 'approvedBy': actorId});

      case ResultStatus.published:
        final isUnlocking = current.status == ResultStatus.locked;
        ...
        if (isUnlocking) {
          ...
          updates.addAll({'publishedAt': nowText, 'publishedBy': actorId});
        }

      case ResultStatus.locked:
        updates.addAll({'lockedAt': nowText, 'lockedBy': actorId});
  ...
  bool _requiresActor(ResultStatus status) {
    return status == ResultStatus.generated ||
