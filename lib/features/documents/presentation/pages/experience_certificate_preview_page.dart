...
enum _EmployeeType { teacher, staff }

class ExperienceCertificatePreviewPage extends StatefulWidget {
  const ExperienceCertificatePreviewPage({super.key});

  @override
  State<ExperienceCertificatePreviewPage> createState() =>
...
class _ExperienceCertificatePreviewPageState
  ...
  late Future<List<TeacherEntity>> _teachersFuture;

  late Future<List<StaffEntity>> _staffFuture;

  _EmployeeType _employeeType = _EmployeeType.teacher;
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              children: [
                ...
                if (_previewReady) _buildPreview(settings),
              ],
            ),
          );
        }
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              SegmentedButton<_EmployeeType>(
                segments: const [
                  ButtonSegment<_EmployeeType>(
                    value: _EmployeeType.teacher,
                    icon: Icon(Icons.school_outlined),
                    label: Text('Teacher'),
                  ),
                  ButtonSegment<_EmployeeType>(
                    value: _EmployeeType.staff,
                ...
                onSelectionChanged: (selection) {
                  ...
                  _changeEmployeeType(selection.first);
                },
              ),
              ...
              InkWell(
                ...
                child: InputDecorator(
                  decoration: InputDecoration(
                    labelText: _employeeType == _EmployeeType.teacher
                        ? 'Select Teacher'
                        : 'Select Staff',
                  ...
                  child: Text(
                    _nameController.text.trim().isEmpty
                        ? _employeeType == _EmployeeType.teacher
                              ? 'Choose teacher'
                              : 'Choose staff member'
              ...
              TextFormField(
                ...
                decoration: InputDecoration(
                  ...
                  hintText: 'DD/MM/YYYY or Present',
  ...
  void _changeEmployeeType(_EmployeeType type) {
    setState(() {
      _employeeType = type;
  ...
  Future<void> _openEmployeePicker() async {
    if (_employeeType == _EmployeeType.teacher) {
  ...
  Future<void> _openTeacherPicker() async {
    ...
    try {
      teachers = await _teachersFuture;
    } catch (e) {
      if (!mounted) {
        return;
      }
    ...
    final activeTeachers =
        teachers.where((teacher) => teacher.isActive).toList()..sort(
          (a, b) =>
              a.fullName.toLowerCase().compareTo(b.fullName.toLowerCase()),
        );

    final selected = await showDialog<TeacherEntity>(
      context: context,
      builder: (_) => _TeacherPickerDialog(teachers: activeTeachers),
    );
  ...
  Future<void> _openStaffPicker() async {
    ...
    try {
      staff = await _staffFuture;
    } catch (e) {
      if (!mounted) {
        return;
      }

      _showError('Unable to load staff: $e');
    ...
    final activeStaff = staff.where((item) => item.isActive).toList()
      ..sort(
        (a, b) => a.fullName.toLowerCase().compareTo(b.fullName.toLowerCase()),
      );

    final selected = await showDialog<StaffEntity>(
      context: context,
      builder: (_) => _StaffPickerDialog(staff: activeStaff),
    );
    ...
    _selectStaff(selected);
  }

  void _selectTeacher(TeacherEntity teacher) {
    setState(() {
      _selectedEmployeeId = teacher.id;

      _nameController.text = teacher.fullName;

      _designationController.text = teacher.designation;

      _joiningDateController.text = _formatDate(teacher.joiningDate);

      _previewReady = false;
    });
  }

  void _selectStaff(StaffEntity staff) {
    setState(() {
      _selectedEmployeeId = staff.id;

      _nameController.text = staff.fullName;

      _designationController.text = staff.designation;

      _joiningDateController.text = _formatDate(staff.joiningDate);
  ...
  Future<void> _pickJoiningDate() async {
    final current = _parseDate(_joiningDateController.text) ?? DateTime.now();
    ...
    setState(() {
      _joiningDateController.text = _formatDate(selected);
  ...
  Future<void> _pickLeavingDate() async {
    final current = _parseDate(_leavingDateController.text) ?? DateTime.now();
  ...
  Widget _buildPreview(SchoolSettingsEntity settings) {
    final template = buildExperienceCertificateTemplateV1();
  ...
  DocumentDataEntity _buildDocumentData() {
    ...
    return DocumentDataEntity(
      ...
      referenceType: _employeeType == _EmployeeType.teacher
          ? 'teacher'
          : 'staff',
      ...
      values: {
        'experience': {
          ...
          'leavingDate': _leavingDateController.text.trim(),
  ...
  Future<Uint8List> _buildPdf() async {
    ...
    return _exportService.createPdfFromPng(
      ...
      title: 'Experience Certificate - ${_nameController.text.trim()}',
    );
  }
  ...
  Future<void> _savePng() async {
    await _runExport(() async {
      final path = await _exportService.savePng(
        ...
        fileName: '${_baseFileName()}_experience_certificate',
      );

      if (path != null) {
        _showSuccess('Experience Certificate PNG saved successfully.');
      }
    });
  }
  ...
  Future<void> _savePdf() async {
    await _runExport(() async {
      ...
      if (path != null) {
        _showSuccess('Experience Certificate PDF saved successfully.');
      }
    });
  }
  ...
  Future<void> _printPdf() async {
    await _runExport(() async {
      await _exportService.printPdf(
        bytes: await _buildPdf(),
        name: 'Experience Certificate - ${_nameController.text.trim()}',
      );
    });
  }
  ...
  Future<void> _sharePdf() async {
    await _runExport(() async {
      await _exportService.sharePdf(
        bytes: await _buildPdf(),
        fileName: '${_baseFileName()}_experience_certificate.pdf',
      );
    });
  }
  ...
  Future<void> _sharePng() async {
    await _runExport(() async {
      await _exportService.sharePng(
        bytes: await _capturePng(pixelRatio: 3),
        fileName: '${_baseFileName()}_experience_certificate.png',
        text: 'Experience Certificate - ${_nameController.text.trim()}',
      );
    });
  }
  ...
  String _baseFileName() {
    return _exportService.safeFileName(
      _nameController.text.trim(),
      fallback: 'experience_certificate',
    );
  }
...
class _TeacherPickerDialogState extends State<_TeacherPickerDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    final results = widget.teachers.where((teacher) {
      if (query.isEmpty) {
        return true;
      }
          ...
          children: [
            TextField(
              ...
              decoration: const InputDecoration(
                ...
                hintText: 'Name, Employee ID or Designation',
            ...
            Expanded(
              child: results.isEmpty
                  ? const Center(child: Text('No matching teacher found.'))
                  : ListView.separated(
...
class _StaffPickerDialog extends StatefulWidget {
  const _StaffPickerDialog({required this.staff});
...
class _StaffPickerDialogState extends State<_StaffPickerDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    final results = widget.staff.where((staff) {
      if (query.isEmpty) {
        return true;
      }
          ...
          children: [
            TextField(
              ...
              decoration: const InputDecoration(
                ...
                hintText: 'Name, Staff ID or Designation',
                prefixIcon: Icon(Icons.search),
                border: OutlineInputBorder(),
              ),
            ),
            ...
            Expanded(
              child: results.isEmpty
                  ? const Center(child: Text('No matching staff member found.'))
                  : ListView.separated(
                      itemCount: results.length,
                      separatorBuilder: (_, _) => const Divider(height: 1),
                      itemBuilder: (context, index) {
                        final staff = results[index];
  ...
  Widget build(BuildContext context) {
    return const Center(
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(Icons.history_edu_outlined, size: 64, color: _textSecondary),
