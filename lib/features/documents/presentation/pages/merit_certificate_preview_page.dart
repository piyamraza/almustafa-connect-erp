...
import '../../templates/merit_certificate/merit_certificate_template_v1.dart';
...
class MeritCertificatePreviewPage extends StatefulWidget {
  const MeritCertificatePreviewPage({
    super.key,
  ...
  @override
  State<MeritCertificatePreviewPage> createState() =>
      _MeritCertificatePreviewPageState();
}

class _MeritCertificatePreviewPageState
    extends State<MeritCertificatePreviewPage> {
  ...
  Widget _buildPreview(
    ...
    final template =
        buildMeritCertificateTemplateV1();
  ...
  Future<Uint8List> _buildPdf() async {
    ...
    if (template.orderedPages.isEmpty) {
      throw StateError(
        'Merit Certificate template has no pages.',
      );
    }
    ...
    return _exportService.createPdfFromPng(
      ...
      title:
          'Merit Certificate - ${widget.result.studentName}',
    );
  }
  ...
  Future<void> _savePng() async {
    await _runExport(() async {
      final path =
        ...
        fileName:
            '${_baseFileName()}_merit_certificate',
      );

      if (path != null) {
        _showSuccess(
          'Merit Certificate PNG saved successfully.',
        );
      }
    });
  }
  ...
  Future<void> _savePdf() async {
    await _runExport(() async {
      ...
      if (path != null) {
        _showSuccess(
          'Merit Certificate PDF saved successfully.',
        );
      }
    });
  }
  ...
  Future<void> _sharePdf() async {
    await _runExport(() async {
      await _exportService.sharePdf(
        bytes: await _buildPdf(),
        fileName:
            '${_baseFileName()}_merit_certificate.pdf',
      );
    });
  }
  ...
  Future<void> _sharePng() async {
    await _runExport(() async {
      await _exportService.sharePng(
        ...
        fileName:
            '${_baseFileName()}_merit_certificate.png',
        text:
            'Merit Certificate - ${widget.result.studentName}',
      );
    });
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              'Unable to load Merit Certificate',
              style: TextStyle(
                color: _textPrimary,
                fontSize: 17,
                fontWeight:
