...
class _StudentIdCardPreviewPageState
    extends State<StudentIdCardPreviewPage> {
  ...
  Future<Uint8List> _buildPdf() async {
    ...
    if (pages.isEmpty) {
      throw StateError(
        'Student ID Card template has no pages.',
      );
    }
    ...
    return _exportService
      ...
      aspectRatio:
          aspectRatio,
      title:
          'Student ID Card - ${widget.student.fullName}',
    );
  }
      ...
      () async {
        ...
        final path =
          ...
          bytes:
              bytes,
          fileName:
              '${_baseFileName()}_student_id_card',
        );

        if (path != null) {
          _showSuccess(
            'Student ID Card PNG saved successfully.',
          );
        }
      },
    );
  ...
  Future<void> _savePdf() async {
    await _runExport(
      () async {
        ...
        if (path != null) {
          _showSuccess(
            'Student ID Card PDF saved successfully.',
          );
        }
      },
    );
      ...
      () async {
        ...
        await _exportService.sharePdf(
          bytes:
              bytes,
          fileName:
              '${_baseFileName()}_student_id_card.pdf',
        );
      },
    );
  }
      ...
      () async {
        ...
        await _exportService.sharePng(
          bytes:
              bytes,
          fileName:
              '${_baseFileName()}_student_id_card.png',
          text:
              'Student ID Card - ${widget.student.fullName}',
        );
      },
